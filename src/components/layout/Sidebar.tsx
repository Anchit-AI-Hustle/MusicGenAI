...
import { Home, Music, LayoutDashboard, LogOut, User, ChevronLeft, ChevronRight, Menu, X } from 'lucide-react';
...
export const Sidebar: React.FC<SidebarProps> = ({ currentPage, onNavigate, onAuthClick }) => {
  ...
  const MobileMenuButton = () => (
    <button
      ...
      className="fixed top-4 left-4 z-40 p-3 rounded-xl bg-sidebar border border-sidebar-border lg:hidden"
    >
  ...
  const SidebarContent = () => (
    ...
    >
      ...
      {isMobile && (
        <button
          onClick={() => setIsMobileOpen(false)}
          className="absolute top-4 right-4 p-2 rounded-lg text-muted-foreground hover:text-foreground hover:bg-sidebar-accent transition-smooth"
        >
          <X className="w-5 h-5" />
        </button>
      )}
      ...
      {!isMobile && (
        <button
          ...
          className="absolute -right-3 top-20 w-6 h-6 rounded-full bg-sidebar border border-sidebar-border flex items-center justify-center hover:bg-sidebar-accent transition-smooth"
        >
          {isCollapsed ? (
        ...
        {isAuthenticated ? (
          <div className="space-y-3">
            ...
            <button
              ...
              className={cn(
                "w-full flex items-center gap-3 px-4 py-2 rounded-lg text-muted-foreground hover:text-destructive hover:bg-destructive/10 transition-smooth",
                isCollapsed && !isMobile && "justify-center px-3"
              )}
            >
          ...
          <button
            ...
            className={cn(
              "w-full flex items-center justify-center gap-2 px-4 py-3 rounded-lg bg-gradient-primary text-primary-foreground font-medium transition-smooth hover:opacity-90 glow-primary",
  ...
  // For mobile: show hamburger + drawer
  if (isMobile) {
    return (
      <>
        <MobileMenuButton />
